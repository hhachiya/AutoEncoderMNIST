	...
	for ((tri=0; tri<3; tri++)); do
		for ((char=0; char<10; char++)); do
			...
			for stopTrainThre in 0.05 0.01; do
				...
				#	python adversarialClassifierAug.py 0 $char $tri $noiseSigma $zDim 10000 $stopTrainThre 1 
					python adversarialClassifierAug.py 0 $char $tri $noiseSigma $zDim 10000 $stopTrainThre 0
				done

				for noiseSigma in 0.0; do
					#python adversarialClassifierAug.py 2 $char $tri $noiseSigma $zDim 10000 $stopTrainThre 1.0
					python adversarialClassifierAug.py 2 $char $tri $noiseSigma $zDim 10000 $stopTrainThre 0.5
